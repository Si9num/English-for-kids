...
import { arrForName, arrForNameRus, arrForSounds } from "./arrsForDataActB";

arrForSounds();
arrForImg.splice(0, arrForImg.length);
arrForImg.push(`open`, `play`, `point`, `ride`, `run`, `sing`, `skip`, `swim`);

const arrForActionBPage: string[] = [];
...
`;
arrForActionBPage.push(btn);
arrForActionBPage.push(btnRep);
export { arrForActionBPage, arrForImg };
